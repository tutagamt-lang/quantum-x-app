...
if candle_data:
    ...
    try: oi_915 = int(df.between_time('09:15', '09:16').iloc[0]['OI_Calc'])
    except Exception: oi_915 = int(df.iloc[0]['OI_Calc'])

    try: oi_930 = int(df.between_time('09:30', '09:31').iloc[0]['OI_Calc'])
    except Exception: oi_930 = int(df.iloc[min(15, len(df)-1)]['OI_Calc'])
...
with tab_live:
    ...
    if current_market_trend == "UpTrend":
        if current_fo_label == "Long Buildup":
            ...
            tamil_desc = f"சந்தை ஏற்றத்தில் உள்ளது மற்றும் புதிய வாங்குதல்கள் (Long Buildup) தொடர்ந்தால், விலை {target_range} உடைத்து மேலே செல்ல வாய்ப்பு அதிகம்."
